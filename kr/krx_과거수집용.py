...
class KrxApiClient:
    ...
    def login(self, session: requests.Session) -> bool:
        ...
        if not krx_id or not krx_pw:
            logger.warning("KRX_ID or KRX_PW not set, skipping login")
            return False
        ...
        try:
            ...
            resp = session.post(KRX_LOGIN_URL, data=login_data)

            if resp.status_code == 200:
                try:
                    json_resp = resp.json()
                    if 'MBR_NO' in json_resp:
                        error_code = json_resp.get('_error_code', '')
                        if error_code == 'CD001':
                            logger.info(f"KRX login successful (MBR_NO: {json_resp['MBR_NO']})")
                            self.logged_in = True
                            return True
                        else:
                            logger.warning(f"KRX login returned code: {error_code}")
                            return False
                    else:
                        logger.error(f"KRX login failed: {json_resp}")
                        return False
                except Exception as e:
                    logger.error(f"Failed to parse login response: {e}")
                    return False
            else:
                logger.error(f"KRX login request failed: {resp.status_code}")
                return False

        except requests.exceptions.RequestException as e:
            logger.error(f"KRX login error: {e}")
            return False
    ...
    def reset_session(self):
        """Reset session and login state for re-authentication"""
        if self.session:
            try:
                self.session.close()
            except:
        ...
        self.session = None
        self.logged_in = False
        logger.info("Session reset for re-authentication")

    def fetch_data(self, payload: Dict[str, Any], max_retries: int = 3) -> Optional[str]:
        """KRX에서 CSV 데이터 다운로드 (자동 재로그인 및 재시도 포함)"""
        last_error = None
        ...
        for attempt in range(max_retries):
            ...
            if not session:
                ...
                if attempt < max_retries - 1:
                    logger.info(f"Retry {attempt + 1}/{max_retries}: Resetting session...")
                    self.reset_session()
...
class KrxCompleteCollector:
    ...
    async def init_pool_for_individual_investor(self, min_size=10, max_size=20):
        ...
        if self.pool is None:
            ...
            self.semaphore = asyncio.Semaphore(5)  # Limit concurrent operations to 5
            logger.info(f"Database pool initialized with min_size={min_size}, max_size={max_size}, semaphore=5")
    ...
    def collect_intraday_data(self, target_date: str = None):
        """KRX 장중 데이터 수집 및 GCS 업로드"""
        csv_data = self.krx_client.get_intraday_data(target_date)
        if not csv_data:
    ...
    async def run_intraday_collection(self, target_date: str = None):
        """장중 데이터 수집 실행"""
        logger.info("Starting KRX intraday data collection (COMPLETE)")

        try:
            success = self.collect_intraday_data(target_date)
            if not success:
    ...
    def collect_intraday_detail_data(self, target_date: str = None):
        """KRX 장중 상세 데이터 수집 및 GCS 업로드"""
        csv_data = self.krx_client.get_intraday_detail_data(target_date)
        if not csv_data:
    ...
    async def run_intraday_detail_collection(self, target_date: str = None):
        """장중 상세 데이터 수집 실행"""
        logger.info("Starting KRX intraday detail data collection (COMPLETE)")

        try:
            success = self.collect_intraday_detail_data(target_date)
            if not success:
    ...
    async def process_intraday_total_data(self, target_date: str = None):
        """kr_intraday와 kr_intraday_detail을 JOIN하여 kr_intraday_total에 저장"""
        database_url = self.database_url
        if database_url.startswith('postgresql+asyncpg://'):
            database_url = database_url.replace('postgresql+asyncpg://', 'postgresql://')
        ...
        try:
            ...
            # kr_intraday와 kr_intraday_detail을 JOIN하여 데이터 가져오기
            query = """
            SELECT
                i.symbol,
                i.stock_name,
            ...
            rows = await conn.fetch(query)
            logger.info(f"Found {len(rows)} records to process for kr_intraday_total")

            if not rows:
                logger.warning("No data found to insert into kr_intraday_total")
                return
            ...
            INSERT INTO kr_intraday_total (
                symbol, stock_name, exchange, close, change_amount, change_rate,
                open, high, low, volume, trading_value, market_cap, listed_shares,
                eps, per, bps, pbr, dps, dividend_yield, date
            ...
            logger.info(f"Inserted/Updated {len(data_to_insert)} kr_intraday_total records")
    ...
    async def run_intraday_total_collection(self, target_date: str = None):
        """장중 통합 데이터 수집 실행"""
        logger.info("Starting KRX intraday total data collection")

        try:
            await self.process_intraday_total_data(target_date)
    ...
    def collect_investor_daily_trading_data(self, target_date: str = None):
        """KRX 투자자별 일별 거래 데이터 수집 및 GCS 업로드"""
        csv_data = self.krx_client.get_investor_daily_trading_data(target_date)
        if not csv_data:
    ...
    async def run_investor_daily_trading_collection(self, target_date: str = None):
        """투자자별 일별 거래 데이터 수집 실행"""
        logger.info("Starting KRX investor daily trading data collection (COMPLETE)")

        try:
            success = self.collect_investor_daily_trading_data(target_date)
            if not success:
    ...
    def collect_program_daily_trading_data(self, target_date: str = None):
        """KRX 프로그램 일별 거래 데이터 수집 및 GCS 업로드"""
        csv_data = self.krx_client.get_program_daily_trading_data(target_date)
        if not csv_data:
    ...
    async def run_program_daily_trading_collection(self, target_date: str = None):
        """프로그램 일별 거래 데이터 수집 실행"""
        logger.info("Starting KRX program daily trading data collection (COMPLETE)")

        try:
            success = self.collect_program_daily_trading_data(target_date)
            if not success:
    ...
    def collect_blocktrades_data(self, target_date: str = None):
        """KRX 대량 거래 데이터 수집 및 GCS 업로드"""
        csv_data = self.krx_client.get_blocktrades_data(target_date)
        if not csv_data:
    ...
    async def run_blocktrades_collection(self, target_date: str = None):
        """대량 거래 데이터 수집 실행"""
        logger.info("Starting KRX block trades data collection (COMPLETE)")

        try:
            success = self.collect_blocktrades_data(target_date)
            if not success:
    ...
    def collect_foreign_ownership_data(self, target_date: str = None):
        """KRX 외국인 지분율 데이터 수집 및 GCS 업로드"""
        csv_data = self.krx_client.get_foreign_ownership_data(target_date)
        if not csv_data:
    ...
    async def run_foreign_ownership_collection(self, target_date: str = None):
        """외국인 지분율 데이터 수집 실행"""
        logger.info("Starting KRX foreign ownership data collection (COMPLETE)")

        try:
            success = self.collect_foreign_ownership_data(target_date)
            if not success:
    ...
    def collect_stock_basic_data(self, target_date: str = None):
        """KRX 주식 기본정보 데이터 수집 및 GCS 업로드"""
        # Note: stock_basic_data does not use target_date, but parameter is kept for consistency
    ...
    async def run_stock_basic_collection(self, target_date: str = None):
        """주식 기본정보 데이터 수집 실행"""
        logger.info("Starting KRX stock basic data collection (COMPLETE)")

        try:
            success = self.collect_stock_basic_data(target_date)
            if not success:
    ...
    def collect_stock_detail_data(self, target_date: str = None):
        """KRX 주식 상세정보 데이터 수집 및 GCS 업로드"""
        # Note: stock_detail_data does not use target_date, but parameter is kept for consistency
    ...
    async def run_stock_detail_collection(self, target_date: str = None):
        """주식 상세정보 데이터 수집 실행"""
        logger.info("Starting KRX stock detail data collection (COMPLETE)")

        try:
            success = self.collect_stock_detail_data(target_date)
            if not success:
    ...
    async def get_individual_investor_data_with_retry(self, symbol_info, trade_date=None, max_retries=5, retry_delay=20):
        """Get individual investor trading data with retry logic (1 retry with 15s delay)"""
        for attempt in range(max_retries):
            try:
    ...
    def get_individual_investor_data(self, symbol_info, trade_date=None):
        ...
        try:
            session = self.krx_client.get_session()
            if not session:
                logger.error(f"No valid session available for symbol {symbol_info['symbol']}")
                return None
    ...
    def parse_individual_investor_csv(self, csv_content, symbol_info, trade_date):
        ...
        try:
            ...
            def safe_int(value):
                try:
                    return int(str(value).replace(',', '')) if value and str(value).strip() != '' else 0
                except:
                    return 0
            ...
            # CSV: [1]=vol_sell, [2]=vol_buy, [3]=vol_net, [4]=val_sell, [5]=val_buy, [6]=val_net
            inst_buy_volume = safe_int(inst_row[2]) if len(inst_row) > 2 else 0
            inst_sell_volume = safe_int(inst_row[1]) if len(inst_row) > 1 else 0
            inst_net_volume = safe_int(inst_row[3]) if len(inst_row) > 3 else 0
            inst_buy_value = safe_int(inst_row[5]) if len(inst_row) > 5 else 0
            inst_sell_value = safe_int(inst_row[4]) if len(inst_row) > 4 else 0
            inst_net_value = safe_int(inst_row[6]) if len(inst_row) > 6 else 0
            inst_buy_ratio = (inst_buy_value / total_buy_value * 100) if total_buy_value > 0 else 0.0

            # Retail data (11th row - index 10)
            retail_buy_volume = safe_int(retail_row[2]) if len(retail_row) > 2 else 0
            retail_sell_volume = safe_int(retail_row[1]) if len(retail_row) > 1 else 0
            ...
            retail_sell_value = safe_int(retail_row[4]) if len(retail_row) > 4 else 0
            retail_net_value = safe_int(retail_row[6]) if len(retail_row) > 6 else 0
            retail_buy_ratio = (retail_buy_value / total_buy_value * 100) if total_buy_value > 0 else 0.0

            # Foreign data (12th row - index 11)
            foreign_buy_volume = safe_int(foreign_row[2]) if len(foreign_row) > 2 else 0
            foreign_sell_volume = safe_int(foreign_row[1]) if len(foreign_row) > 1 else 0
            ...
            foreign_sell_value = safe_int(foreign_row[4]) if len(foreign_row) > 4 else 0
            foreign_net_value = safe_int(foreign_row[6]) if len(foreign_row) > 6 else 0
            foreign_buy_ratio = (foreign_buy_value / total_buy_value * 100) if total_buy_value > 0 else 0.0

            # Total data (14th row - index 13)
            total_buy_volume = safe_int(total_row[2]) if len(total_row) > 2 else 0
            total_sell_volume = safe_int(total_row[1]) if len(total_row) > 1 else 0
            ...
            total_sell_value = safe_int(total_row[4]) if len(total_row) > 4 else 0
            total_net_value = safe_int(total_row[6]) if len(total_row) > 6 else 0
    ...
    def get_progress_file_path(self):
        """Get progress file path for individual investor collection (single file for all dates)"""
        ...
        return f"{log_dir}/individual_investor_progress.txt"

    def load_completed_symbols(self, target_date: str = None):
        """Load completed symbols from progress file for specific date"""
    ...
    def save_completed_symbol(self, symbol, target_date: str = None):
        """Save completed symbol to progress file with date"""
        progress_file = self.get_progress_file_path()
        try:
            # Save in format: date,symbol
            with open(progress_file, 'a', encoding='utf-8') as f:
                if target_date:
                    f.write(f"{target_date},{symbol}\n")
                else:
                    # Use current date if not provided
                    current_date = datetime.now().strftime('%Y%m%d')
                    f.write(f"{current_date},{symbol}\n")
        except Exception as e:
    ...
    async def process_single_symbol_investor_data(self, symbol_info, trade_date):
        ...
        async with self.semaphore:
            try:
                ...
                if parsed_data:
                    # Save progress immediately after successful processing (with target_date)
                    self.save_completed_symbol(symbol_info['symbol'], trade_date)
                    return parsed_data
                else:
                    return None
    ...
    async def process_individual_investor_daily_trading_data(self, target_date: str = None):
        ...
        # Load completed symbols from progress file (using target_date)
        completed_symbols = self.load_completed_symbols(target_date)
        ...
        if not symbols:
            ...
            await self.close_pool()
            # Return statistics even when all completed
            return {
                'total_symbols': len(all_symbols),
                'successful': 0,
                'failed': 0,
                'already_completed': len(completed_symbols),
                'completed': len(completed_symbols)
            }

        # Fixed batch size for stable performance (middle-ground optimization)
        batch_size = 20  # Reduced to avoid KRX rate limiting
        ...
        # Return statistics for caller to check
        return {
            'total_symbols': len(all_symbols),
            'successful': total_successful,
            'failed': total_failed,
            'already_completed': len(completed_symbols),
            'completed': len(completed_symbols) + total_successful
        }
    ...
    async def run_individual_investor_daily_trading_collection(self, target_date: str = None):
        ...
        try:
            result = await self.process_individual_investor_daily_trading_data(target_date)
