        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if not download_code:
                    ...
                    if attempt < max_retries - 1:
                        logger.info(f"Retry {attempt + 1}/{max_retries}: Re-authenticating...")
                        self.reset_session()
                ...
                if attempt < max_retries - 1:
                    logger.info(f"Retry {attempt + 1}/{max_retries}: Re-authenticating after error...")
                    self.reset_session()
                    time.sleep(2 ** attempt)
                    continue
                return None
...
class KrxCompleteCollector:
    ...
    async def init_pool_for_individual_investor(self, min_size=10, max_size=20):
        ...
        if self.pool is None:
            ...
            self.semaphore = asyncio.Semaphore(5)  # Limit concurrent operations to 5 (KRX-friendly)
    ...
    async def process_individual_investor_daily_trading_data(self):
        ...
        try:
            query = """
            SELECT sb.symbol, sb.standard_symbol, sb.stock_name
            FROM kr_stock_basic sb
            INNER JOIN kr_intraday ki ON sb.symbol = ki.symbol
            WHERE sb.symbol IS NOT NULL
              AND sb.standard_symbol IS NOT NULL
              AND ki.volume > 0
            ORDER BY sb.symbol
            """
            rows = await conn.fetch(query)
