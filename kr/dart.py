...
def _determine_dart_target_quarter():
    """현재 시즌에 맞는 DART 타겟 분기/연도 결정"""
    current_month = datetime.now().month
    current_year = datetime.now().year

    if current_month in [1, 2, 3]:
        return current_year - 1, '11011', 'Q4(사업보고서)'
    elif current_month in [4, 5, 6]:
        return current_year, '11013', 'Q1(1분기)'
    elif current_month in [7, 8, 9]:
        return current_year, '11012', 'Q2(반기)'
    else:
        return current_year, '11014', 'Q3(3분기)'


def _check_dart_data_available(api_key, target_year, target_report_code):
    """DART API 1건으로 데이터 존재 여부 확인 (삼성전자 재무제표 기준)"""
...
def _get_stlm_dt_range(target_year, target_report_code):
    """결산기준일 범위 반환 (분기별)"""
    if target_report_code == '11013':      # Q1
        return date(target_year, 3, 1), date(target_year, 3, 31)
    elif target_report_code == '11012':    # Q2/반기
        return date(target_year, 6, 1), date(target_year, 6, 30)
    elif target_report_code == '11014':    # Q3
        return date(target_year, 9, 1), date(target_year, 9, 30)
    else:                                  # Q4/사업보고서
        return date(target_year, 12, 1), date(target_year, 12, 31)
...
class DartFinancialManager:
    ...
    async def pre_check_should_collect(self, companies):
        """
        사전 체크: DART 최신 분기 vs DB 최신 분기 비교
        DART API 1건 + DB 쿼리 1건으로 수집 필요 여부를 빠르게 판단
        Returns: (should_collect: bool, reason: str)
        """
        target_year, target_report_code, quarter_name = _determine_dart_target_quarter()
        logger.info(f"Pre-check [financial_position]: 타겟 = {target_year}년 {quarter_name}")

        # Step 1: DART 데이터 존재 여부 확인 (삼성전자 재무제표 API 1건)
        if not _check_dart_data_available(self.api_key, target_year, target_report_code):
            return False, f"DART에 {target_year}년 {quarter_name} 데이터 아직 없음 (삼성전자 기준)"

        logger.info(f"Pre-check: DART에 {target_year}년 {quarter_name} 데이터 확인됨")

        # Step 2: DB에 해당 분기 수집 현황 확인 (DB 쿼리 1건)
        async with await self.get_connection() as conn:
            db_count = await conn.fetchval("""
                SELECT COUNT(DISTINCT corp_code)
                FROM kr_financial_position
                WHERE bsns_year = $1 AND report_code = $2
            """, target_year, target_report_code)

        total_target = len(companies)
        coverage = (db_count / total_target * 100) if total_target > 0 else 0
        logger.info(f"Pre-check: DB 현황 = {db_count}/{total_target} ({coverage:.1f}%)")

        # Step 3: 95% 이상이면 수집 완료로 간주
        if total_target > 0 and db_count >= total_target * 0.95:
            return False, f"DB에 {target_year}년 {quarter_name} 이미 충분 ({db_count}/{total_target}, {coverage:.1f}%)"

        return True, f"{target_year}년 {quarter_name} 수집 필요 (DB: {db_count}/{total_target}, {coverage:.1f}%)"
    ...
    async def run_collection(self):
        ...
        # 사전 체크: DART 최신 분기 vs DB 최신 분기 비교 (API 1건 + DB 쿼리 1건)
        should_collect, reason = await self.pre_check_should_collect(companies)
        if not should_collect:
            logger.info(f"Collection skipped: {reason}")
...
class DartStockAcquisitionCollector:
    ...
    async def pre_check_should_collect(self, total_companies):
        """사전 체크: DART 최신 분기 vs DB 비교 (API 1건 + DB 쿼리 1건)"""
        target_year, target_report_code, quarter_name = _determine_dart_target_quarter()
        logger.info(f"Pre-check [stockacquisition]: 타겟 = {target_year}년 {quarter_name}")
        ...
        dt_start, dt_end = _get_stlm_dt_range(target_year, target_report_code)
        async with await self.get_connection() as conn:
            db_count = await conn.fetchval("""
                SELECT COUNT(DISTINCT corp_code) FROM kr_stockacquisitiondisposal
                WHERE stlm_dt >= $1 AND stlm_dt <= $2
            """, dt_start, dt_end)

        coverage = (db_count / total_companies * 100) if total_companies > 0 else 0
        logger.info(f"Pre-check: DB 현황 = {db_count}/{total_companies} ({coverage:.1f}%)")

        if total_companies > 0 and db_count >= total_companies * 0.95:
            return False, f"DB에 {target_year}년 {quarter_name} 이미 충분 ({db_count}/{total_companies}, {coverage:.1f}%)"

        return True, f"{target_year}년 {quarter_name} 수집 필요 (DB: {db_count}/{total_companies}, {coverage:.1f}%)"
    ...
    async def collect_stockacquisition_data(self):
        ...
        # 사전 체크
        should_collect, reason = await self.pre_check_should_collect(len(companies))
        if not should_collect:
            logger.info(f"Collection skipped: {reason}")
            await self.close_pool()
            return
...
class DartAuditCollector:
    ...
    async def pre_check_should_collect(self, total_companies):
        """사전 체크: DART 최신 분기 vs DB 비교 (API 1건 + DB 쿼리 1건)"""
        target_year, target_report_code, quarter_name = _determine_dart_target_quarter()
        logger.info(f"Pre-check [audit]: 타겟 = {target_year}년 {quarter_name}")
        ...
        async with await self.get_connection() as conn:
            db_count = await conn.fetchval("""
                SELECT COUNT(DISTINCT corp_code) FROM kr_audit
                WHERE bsns_year = $1
            """, str(target_year))
...
class DartDividendsCollector:
    ...
    async def pre_check_should_collect(self, total_companies):
        """사전 체크: DART 최신 분기 vs DB 비교 (API 1건 + DB 쿼리 1건)"""
        target_year, target_report_code, quarter_name = _determine_dart_target_quarter()
        logger.info(f"Pre-check [dividends]: 타겟 = {target_year}년 {quarter_name}")
        ...
        async with await self.get_connection() as conn:
            db_count = await conn.fetchval("""
                SELECT COUNT(DISTINCT corp_code) FROM kr_dividends
                WHERE stlm_dt >= $1 AND stlm_dt <= $2
            """, dt_start, dt_end)
...
class DartShareholderCollector:
    ...
    async def pre_check_should_collect(self, total_companies):
        """사전 체크: DART 최신 분기 vs DB 비교 (API 1건 + DB 쿼리 1건)"""
        target_year, target_report_code, quarter_name = _determine_dart_target_quarter()
        logger.info(f"Pre-check [shareholder]: 타겟 = {target_year}년 {quarter_name}")
        ...
        async with await self.get_connection() as conn:
            db_count = await conn.fetchval("""
                SELECT COUNT(DISTINCT corp_code) FROM kr_largest_shareholder
                WHERE stlm_dt >= $1 AND stlm_dt <= $2
            """, dt_start, dt_end)
...
class DartExecutiveCollector:
    ...
    async def pre_check_should_collect(self, total_companies):
        """사전 체크: DART 최신 분기 vs DB 비교 (API 1건 + DB 쿼리 1건)"""
        target_year, target_report_code, quarter_name = _determine_dart_target_quarter()
        logger.info(f"Pre-check [executive]: 타겟 = {target_year}년 {quarter_name}")
        ...
        async with await self.get_connection() as conn:
            db_count = await conn.fetchval("""
                SELECT COUNT(DISTINCT corp_code) FROM kr_executive
                WHERE stlm_dt >= $1 AND stlm_dt <= $2
            """, dt_start, dt_end)
