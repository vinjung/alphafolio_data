...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("한국은행 경제지표 데이터 수집 시작")
    print("=" * 60)

    # 환경 변수에서 API 키와 DB URL 가져오기
    BOK_API_KEY = os.getenv('BOK_API_KEY')
    DATABASE_URL = os.getenv('DATABASE_URL')

    if not BOK_API_KEY:
        print("Error: BOK_API_KEY 환경변수가 설정되지 않았습니다.")
        exit(1)
    ...
    # 수집 모드 선택 (기본값: incremental)
    mode = os.getenv('BOK_MODE', 'incremental')

    print(f"수집 모드: {mode}")
    print(f"API 키: {BOK_API_KEY[:10]}...")
    print(f"DB 연결: {DATABASE_URL[:30]}...")
    print()

    # 데이터 수집 실행
    start_time = datetime.now()
    result = collect_bok_indicators(
        api_key=BOK_API_KEY,
    ...
    print(f"수집 모드: {result.get('mode', 'N/A')}")
    print(f"총 저장 레코드: {result['total_saved']}")
    print(f"소요 시간: {duration:.2f}초")
    print()

    if result['indicators']:
        print("지표별 수집 내역:")
        for stat_code, info in result['indicators'].items():
            print(f"  - {info['name']} ({stat_code})")
            print(f"    저장: {info['saved']}건")
            print(f"    기간: {info['period']}")
